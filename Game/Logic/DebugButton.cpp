...
void DebugButton::Initialize(GameApp& app, const std::wstring& text, const std::string& name, const Vector2& position, const Vector2& scale) {
    name_ = name;
    isPressed_ = false;
...
void DebugButton::Update(GameApp& app, const Matrix4x4& view, const Matrix4x4& proj) {
...
// ★ 追加：DrawImGuiの実装
void DebugButton::DrawImGui() {
    // ボタン個別の識別名でツリーを展開
    std::string label = "DebugButton: " + name_;
