...
void Card3D::SetCardData(const CardDef& def, const CardInstance& inst)
{
	...
	switch (inst.suit) {
	case CardSuit::Spade:
		suitModelPath = "cards/models/spade.obj";
		suitObj_->SetMaterialColor({ 0.f,0.f,0.f,1.f }); // 黒
		numberObjOnes_->SetMaterialColor({ 0.f,0.f,0.f,1.f });
		if (hasTensDigit_) {
			numberObjTens_->SetMaterialColor({ 0.f,0.f,0.f,1.f });
		}

		break;
	case CardSuit::Heart:
		suitModelPath = "cards/models/heart.obj";
		suitObj_->SetMaterialColor({ 1.f,0.f,0.f,1.f }); // 赤色
		numberObjOnes_->SetMaterialColor({ 1.f,0.f,0.f,1.f });
		if (hasTensDigit_) {
			numberObjTens_->SetMaterialColor({ 1.f,0.f,0.f,1.f });
		}

		break;
	case CardSuit::Diamond:
		suitModelPath = "cards/models/daiya.obj";
		suitObj_->SetMaterialColor({ 0.529f, 0.808f, 0.922f, 1.0f }); // 空色
		numberObjOnes_->SetMaterialColor({ 0.529f, 0.808f, 0.922f,1.f });
		if (hasTensDigit_) {
			numberObjTens_->SetMaterialColor({ 0.529f, 0.808f, 0.922f,1.f });
		}

		break;
	case CardSuit::Club:
		suitModelPath = "cards/models/clover.obj";
		suitObj_->SetMaterialColor({ 0.294f, 0.0f, 0.510f, 1.0f }); // 紫
		numberObjOnes_->SetMaterialColor({ 0.294f, 0.0f, 0.510f, 1.0f });
		if (hasTensDigit_) {
			numberObjTens_->SetMaterialColor({ 0.294f, 0.0f, 0.510f, 1.0f });
		}
