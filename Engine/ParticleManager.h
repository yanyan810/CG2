...
// 定数
// ===============================
static constexpr uint32_t kMaxInstance = 10000;
// 板ポリ1枚 = 2三角形 = 6頂点
...
public:
    ...
    void Initialize(DirectXCommon* dxCommon, SrvManager* srvManager, ParticleCommon* particleCommon); 
    void Finalize();
    void SetGroupBlendMode(const std::string& groupName, ParticleCommon::BlendMode mode);
    ...
    void EmitAll(const Vector3& pos, uint32_t count = 0);

    // エフェクト単位でのロードとエミット（複数のJSONを同時に扱うため）
    void LoadEffect(const std::string& effectName, const std::string& filename);
    void LoadAllEffects(); // Resources/Particles/ 内の全JSONを自動ロード
    void EmitEffect(const std::string& effectName, const Vector3& pos, uint32_t count = 0);

    // エディタ等からのアクセス用
    std::unordered_map<std::string, ParticleGroup>& GetParticleGroups() { return particleGroups_; }
    static Model* GetOrMakeParticlePrimitiveModel(int typeIndex);
